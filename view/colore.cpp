...
ColoreRGB& ColoreRGB::operator =(const ColoreRGB &c)
{
    red = c.red;
    blue = c.blue;
    green = c.green;
...
ColoreXYZ& ColoreXYZ::operator =(const ColoreXYZ &c)
{
    x = c.x;
    y = c.y;
    z = c.z;
...
ColoreCMY& ColoreCMY::operator =(const ColoreCMY &c)
{
    cyan = c.cyan;
    magenta = c.magenta;
    yellow = c.yellow;
...
ColoreCMYK& ColoreCMYK::operator =(const ColoreCMYK &c)
{
    setCyan(c.getCyan());
    setMagenta(c.getMagenta());
    setYellow(c.getYellow());
    black = c.black;
    return *this;
}
