...
Stringa::Stringa(char *c,  int _dim)
{
    //if(_dim>0)
    //{
    dim = _dim;
...
void Stringa::remove( int id,  int q)
{
    dim-=q;
    char *c = new char[dim]();
    for( int i = 0; i<dim; i++)
...
Stringa Stringa::subString( int start, int len) const
{
